...
public class CustomerManagementMockImpl implements CustomerManagementService {
    ...
    public List<Customer> findCustomersByName(String name) {
        return customerMap.values().stream().filter(customer -> customer.getCompanyName()
                .equalsIgnoreCase(name)).collect(Collectors.toList());
    }

    @Override
    public List<Customer> getAllCustomers() {
        List<Customer> allCustomers = new ArrayList<>(customerMap.values());
    ...
    public Customer getFullCustomerDetail(String customerId) throws CustomerNotFoundException {
        Customer customerDetails = customerMap.get(customerId);
